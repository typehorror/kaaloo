...
def gravatar_img_for_user(user, size=80, css_class=""):
    user = get_user(user)
    url = gravatar_for_user(user)
    return """<img src="%s" alt="Avatar for %s" height="%s" width="%s" class="%s"/>""" % (escape(url), user.username, size, size, css_class)

def gravatar(user, size=80, css_class=""):
    # backward compatibility
    return gravatar_img_for_user(user, size, css_class)
